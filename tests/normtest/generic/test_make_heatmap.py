"""Tests if  ``make_heatmap`` is working as expected

--------------------------------------------------------------------------------
Command to run at the prompt:
    python -m unittest -v tests/normtest/generic/test_make_heatmap.py
    or
    python -m unittest -b tests/normtest/generic/test_make_heatmap.py
...
### FUNCTION IMPORT ###
from normtest.normtest import make_heatmap
from normtest import normtest as nm
from tests.functions_to_test import functions

os.system('cls')

class Test_make_heatmap(unittest.TestCase):
    ...
    def test_outputs(self):
        fig, ax = plt.subplots()
        result = make_heatmap(ax, self.df_data, self.n_samples)
    ...
    def test_safe(self):
        fig, ax = plt.subplots()
        result = make_heatmap(ax, self.df_data, self.n_samples, safe=True)
        self.assertIsInstance(result[0], SubplotBase, msg="not a SubplotBase")
        self.assertIsInstance(result[1], pd.DataFrame, msg="not a dataframe")
        plt.close()
    ...
    def test_basic_plot(self):
        ...
        fig1_base_path = Path("tests/normtest/generic/figs_make_heatmap/heat_map_42.png")

        fig, ax = plt.subplots(figsize=(14,4)) 
        result = make_heatmap(ax, self.df_data, self.n_samples)
        fig1_file = Path("tests/normtest/generic/figs_make_heatmap/fig1_test.png")
