...
def make_skew_kurtosis_plot(axes, data_frame, n_rep_column_name=None, id_column_name=None, alpha_column_name=None, skewness_column_name=None, kurtosis_column_name=None, test_column_name=None, normal=True, safe=False):
    ...
    axes : matplotlib.axes.SubplotBase
        The axes to plot.    
    data_frame : :doc:`DataFrame <pandas:reference/api/pandas.DataFrame>`
        A dataframe with at least six (``6``) columns containing the number of repetitions, an identifier, the kurtosis, the skewness and the result of a normality test for each dataset.
    ...
    alpha_column_name : str, optional
        The name of the column containing the significance level adopted for each test. If *None*, the name of the third column of the *data_frame* is assigned to this parameter.                                
    skewness_column_name : str, optional
        The name of the column containing the skewness for each dataset. If *None*, the name of the fourth column of the *data_frame* is assigned to this parameter.
    kurtosis_column_name : str, optional
        The name of the column containing the kurtosis. If *None*, the name of the fifth column of the *data_frame* is assigned to this parameter.
    test_column_name : str, optional
        The name of the column containing the test result as *bool*. If *None*, the name of the sixth column of the *data_frame* is assigned to this parameter.        
    normal : bool, optional
        Whether to consider the data truly coming from the Normal distribution (*True*, default) or not (*False*). 
    ...
    Notes
    -----
    If *n_rep_column_name* or *id_column_name* or *alpha_column_name* or *kurtosis_column_name* or *skewness_column_name* or *test_column_name* are *None*, they are all interpreted as *None*.
    ...
    if safe:
        checkers._check_is_subplots(axes, "axes")
        checkers._check_is_data_frame(data_frame, "data_frame")
        if data_frame.shape[0] < 6:
            try:
                raise ValueError("Missing columns error")
            except ValueError:
                print(f"\n\nThe data frame 'data_frame' must contain at least 6 columns, but it only contains {data_frame.shape[0]}.\n\n")
                raise            
        if not (n_rep_column_name is None or id_column_name is None or alpha_column_name is None or kurtosis_column_name is None or skewness_column_name is None or test_column_name is None):
    ...
    if n_rep_column_name is None or id_column_name is None or alpha_column_name is None or kurtosis_column_name is None or skewness_column_name is None or test_column_name is None:
        n_rep_column_name = data_frame.columns[0]
        id_column_name = data_frame.columns[1]
        alpha_column_name = data_frame.columns[2]
...
def skew_kurtosis_plot(axes, data_frame, test_column_name=None, skewness_column_name=None, kurtosis_column_name=None, reverse=True, palette_color="red", marker_size=50, safe=False):
    ...
    test_column_name : str, optional
        The name of the column containing the test result as percentage. If *None*, the name of the first column of the *data_frame* is assigned to this parameter.        
    skewness_column_name : str, optional
        The name of the column containing the skewness. If *None*, the name of the second column of the *data_frame* is assigned to this parameter.
    kurtosis_column_name : str, optional
        The name of the column containing the kurtosis. If *None*, the name of the third column of the *data_frame* is assigned to this parameter.
    ...
    if safe:
        ...
        if data_frame.shape[0] < 3:
            try:
                ...
                print(f"\n\nThe data frame 'data_frame' must contain at least 3 columns, but it only contains {data_frame.shape[0]}.\n\n")
                raise            
        if not (skewness_column_name is None or kurtosis_column_name is None or test_column_name is None):
            checkers._check_is_str(skewness_column_name, "skewness_column_name")
            checkers._check_is_str(kurtosis_column_name, "kurtosis_column_name")
            checkers._check_is_str(test_column_name, "test_column_name")
    ...
    if skewness_column_name is None or kurtosis_column_name is None or test_column_name is None:
        test_column_name = data_frame.columns[0]
        skewness_column_name = data_frame.columns[1]
